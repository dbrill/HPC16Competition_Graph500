		...
		int64_t nv, const int64_t * restrict bfs_tree, int64_t root)
{
  ...
  OMP("omp parallel shared(err)") {
    ...
    OMP("omp for")
				//iterate through number of vertexes
      for (k = 0; k < nv; ++k)
			//if k is not the root vertex intialize it to unvisited (-1)
	level[k] = (k == root? 0 : -1);

    OMP("omp for") MTA("mta assert parallel") MTA("mta use 100 streams")
      for (k = 0; k < nv; ++k) {
				//if k has been visited head to next k
	if (level[k] >= 0) continue;
	terr = err;
	if (!terr && *(ptr + k)->value >= 0 && k != root) {
	  int64_t parent = k;
	  int64_t nhop = 0;
	  /* Run up the tree until we encounter an already-leveled vertex. */
	  while (parent >= 0 && level[parent] < 0 && nhop < nv) {
	    assert (parent != (ptr + parent)->value );
	    parent = (bfs_tree + parent)->value;
	    ++nhop;
	  }
		 ...
		 const struct packed_edge *IJ_in, int64_t nedge)
{
  ...
  */
  ...
  if (root > max_bfsvtx || (ptr + root)->value != root)
    return -999;
