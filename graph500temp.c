...
if (!dumpname) {
	...
	//Process is root, then generate the edgelist and broasdcast it to the othe processes
	//procecss is not root, then listen for edgelist IJ from root, 0
	if(rank ==0)
	{
		fprintf (stderr, " Bcast1 done.\n");	
		fprintf (stderr, " Bcast2 %d, nedge %d.\n", rank, nedge);
		MPI_Bcast(IJ, nedge/2, MPI_CHAR, 0, MPI_COMM_WORLD);
		fprintf (stderr, " Bcast2 done.\n");	
	} else {
		int ntemp;
		MPI_Status status;
		fprintf (stderr, " Recv2 %d, nedge %d.\n", rank, nedge);
		MPI_Bcast(IJ, nedge/2, MPI_CHAR,0, MPI_COMM_WORLD);
		//MPI_Recv(IJ, nedge/2, MPI_CHAR, 0, 123, MPI_COMM_WORLD, &status);
		fprintf (stderr, " Recv2 done.\n");	

	}
  } else {
    int fd;
    ssize_t sz;
	fprintf (stderr, "dumpname: %d\n", dumpname);
...
run_bfs (void)
{
 ...
 // if(rank == 0){
   //Process is root, then generate the edgelist
